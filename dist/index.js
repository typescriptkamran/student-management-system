...
class Student {
    ...
    studentClass;
    section;
    constructor(name, rollNo, age, grade, studentClass, section) {
        this.name = name;
        this.rollNo = rollNo;
        this.age = age;
        this.grade = grade;
...
function saveDataToFile(students) {
    fs.writeFileSync(dataFilePath, JSON.stringify(students, null, 2), 'utf8');
}
async function main() {
    let students = readDataFromFile();
    function displayStudents() {
        console.log(chalk.bold('List of Students:\n'));
        students.forEach((student, index) => {
            console.log(`${index + 1}. Name: ${student.name}, Roll No: ${student.rollNo}, Age: ${student.age}, Grade: ${student.grade}, Class: ${student.studentClass}, Section: ${student.section}`);
        });
        console.log('\n');
    }
    async function addStudent() {
        ...
        const newStudent = new Student(answers.name, answers.rollNo, parseInt(answers.age), answers.grade, answers.studentClass, answers.section);
    ...
    function editStudent() {
        inquirer
            ...
            inquirer
                ...
                .then((editAnswers) => {
                const updatedStudent = new Student(editAnswers.name, editAnswers.rollNo, parseInt(editAnswers.age), editAnswers.grade, editAnswers.studentClass, editAnswers.section);
